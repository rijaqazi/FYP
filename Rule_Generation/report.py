...
def generate_report(rule_json_path, outdir="reports", hours=24, ipinfo_token=None):
    ...
    folder = os.path.join(outdir, rule_id)
    ...
    pdf_path = os.path.join(folder, f"{rule_id}-report.pdf")
    HTML(string=html).write_pdf(pdf_path)
    dst_json = os.path.join(folder, os.path.basename(rule_json_path))
    copy2(rule_json_path, dst_json)
    zip_path = os.path.join(folder, f"{rule_id}.zip")
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.write(dst_json, arcname=os.path.basename(dst_json))
        zf.write(pdf_path, arcname=os.path.basename(pdf_path))
    print(f"[+] PDF report created: {pdf_path}")
    return {"pdf": pdf_path, "zip": zip_path, "folder": folder}
