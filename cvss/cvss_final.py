...
def process_log_file(file_path):
    ...
    with open(file_path, "r") as f:
        for line in f:
            # New unified format already starts with [ALERT] TYPE from IP ...
            match = re.search(r"\[ALERT\]\s+([A-Z0-9_ ]+)\s+from\s+([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)", line, re.IGNORECASE)
            if match:
...
if path.exists():
    all_alerts.extend(process_log_file(path))
else:
