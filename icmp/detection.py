...
def unified_log_alert(alerts_list, alerted_set, log_file_handle, alert_type, src_ip,
    ...
    """
    Produce the same '[ALERT] ...' string format (red printed),
    write to log file (if provided), and append a JSON-friendly dict to alerts_list.
...
def detect_arp(packets, alerts_list, alerted_set, log_file_handle):
    ...
    for packet in arp_packets:
        try:
            ...
            # Broadcast Spoof
            if claimed_mac and claimed_mac.lower() == "ff:ff:ff:ff:ff:ff" or dst_ip == "255.255.255.255":
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
            ...
            if src_ip == dst_ip and arp_table.get(dst_ip) and arp_table[dst_ip] != src_mac:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                  "CACHE_OVERRIDE", src_ip, dst_ip, src_mac, claimed_mac,
    ...
    for pkt in icmp_packets:
        ...
        if icmp_type == "8":
            ...
            if len(times['echo_flood'][src_ip]) >= THRESHOLDS['echo_flood']:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                  "ICMP_Echo_Request_Flood", src_ip,
        ...
        if icmp_type == "8" and dst_ip.endswith(".255"):
            ...
            if len(times['smurf'][src_ip]) >= THRESHOLDS['smurf']:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                  "Smurf_Attack", src_ip, dst_ip, src_mac,
                                  ports="", total_ports=0,
        ...
        if icmp_type == "13":
            ...
            if len(times['timestamp_flood'][src_ip]) >= THRESHOLDS['timestamp_flood']:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                  "ICMP_Timestamp_Request_Flood", src_ip, dst_ip, src_mac,
                                  ports="", total_ports=0,
        ...
        if icmp_type == "17":
            ...
            if len(times['mask_flood'][src_ip]) >= THRESHOLDS['mask_flood']:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                  "ICMP_Address_Mask_Request_Flood", src_ip, dst_ip, src_mac,
                                  ports="", total_ports=0,
        ...
        if icmp_type and (more_frag == '1' or frag_offset > 0):
            ...
            if len(times['fragment_flood'][key]) >= THRESHOLDS['fragment_flood']:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                  "ICMP_Fragmentation_Flood", src_ip, dst_ip, src_mac,
                                  ports="", total_ports=0,
