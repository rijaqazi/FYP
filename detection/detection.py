...
def detect_arp(packets, alerts_list, alerted_set, log_file_handle):
    ...
    for packet in arp_packets:
        try:
            ...
            # === Gratuitous ARP Detection (Fixed) ===
            # Detects when a host announces itself (src == dst) or advertises its own MAC
            if src_ip == dst_ip and claimed_mac == src_mac and timestamps[src_ip]:
                unified_log_alert(alerts_list, alerted_set, log_file_handle,
...
def detect_icmp(packets, alerts_list, alerted_set, log_file_handle):
    ...
    FLOOD_WINDOW = 5
    THRESHOLDS = {
        'echo_flood': 50, 'smurf': 20, 'timestamp_flood': 20, 'mask_flood': 15, 'fragment_flood': 15  # Lowered fragment_flood threshold
    }
    times = {
    ...
    for pkt in packets:  # Process all packets for fragmentation
        try:
            ...
            ip_id = ip_layer.get("ip.id", "")

            if src_mac != "N/A":
                ip_to_mac[src_ip] = src_mac

            # Check for fragmented ICMP packets
            more_fragments = ip_layer.get("ip.flags.mf", '0') == '1'
            frag_offset = int(ip_layer.get("ip.frag_offset", '0')) > 0
            is_fragment = (more_fragments or frag_offset) and 'icmp' in layers

            if is_fragment and src_ip and ip_id and ts:
                key = (src_ip, dst_ip, ip_id)
                fragment_packets[key].append(ts)
                fragment_packets[key] = [t for t in fragment_packets[key] if ts - t <= FLOOD_WINDOW]

                total_fragments = sum(len(v) for k, v in fragment_packets.items() if k[0] == src_ip and k[1] == dst_ip)
                if total_fragments >= THRESHOLDS['fragment_flood'] and 'ICMP FRAGMENTATION FLOOD' not in alerted_ips[src_ip]:
                    src_timestamps = [t for k, v in fragment_packets.items() if k[0] == src_ip and k[1] == dst_ip for t in v]
                    if src_timestamps:
                        min_ts = min(src_timestamps)
                        max_ts = max(src_timestamps)
                        duration = max_ts - min_ts if len(src_timestamps) > 1 else 0
                        unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                          "ICMP FRAGMENTATION FLOOD", src_ip, dst_ip,
                                          ip_to_mac[src_ip], ports="", total_ports=0,
                                          start_time=time.ctime(min_ts),
                                          duration=duration,
                                          packet_count=total_fragments)
                        alerted_ips[src_ip].add('ICMP FRAGMENTATION FLOOD')
                        logger.info(f"Fragmentation flood detected: {total_fragments} fragments from {src_ip} to {dst_ip}")

        except Exception as e:
            logger.error(f"Fragment processing error: {str(e)}")
    ...
    for pkt in icmp_packets:
        try:
            ...
            if icmp_type == "13":
                ...
                if len(times['timestamp_flood'][src_ip]) >= THRESHOLDS['timestamp_flood'] and 'ICMP TIMESTAMP REQUEST FLOOD' not in alerted_ips[src_ip] and times['timestamp_flood'][src_ip]:
                    unified_log_alert(alerts_list, alerted_set, log_file_handle,
                                      ...
                                      packet_count=len(times['timestamp_flood'][src_ip]))
                    alerted_ips[src_ip].add('ICMP TIMESTAMP REQUEST FLOOD')
                    logger.info(f"Timestamp flood detected: {len(times['timestamp_flood'][src_ip]) } packets from {src_ip}")
